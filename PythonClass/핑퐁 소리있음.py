...
import random
import time
import pygame   # 패들에 공이 닿아서 튕길 때 소리를 출력하기 위해 파이게임 모듈을 불러옵니다
...
class Ball:
    ...
    def __init__(self, canvas, paddle, color):
        ...
        self.sound_setting()    # 사운드를 불러오는 함수를 불러옵니다

    def sound_setting(self):    # 사운드를 불러오는 함수입니다
        pygame.init()   # 파이게임 모듈을 불러옵니다
        self.hit_sound = pygame.mixer.Sound("hit.wav")  # 파이게임 모듈을 사용해서 hit.wav 파일을 재생하게 합니다
    ...
    def draw(self):
        ...
        if self.hit_paddle(pos) == True: #판에 부딪히면 위로 튕겨올라가게
            self.hit_sound.play()   # 만약 판에 부딫히는 경우 소리를 재생하게 합니다
            self.y = -3
