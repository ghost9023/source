...
1장 4절 N 아이템의 최대 최소값 찾기 : 컬렉션 내부에서 가장 크거나 작은 N개의 아이템을 찾아야 할때 heapq 모듈에 있는 nlargest()와 nsmallest() 함수를 사용한다.
    ...
    nlargest와 nsmallest는 아이템의 갯수가 적을 때 사용하는 것이 좋다. 아이템이 많을 경우 최대값이나 최소값을 구할 땐 min()과 max()를 사용하는 것이 더 빠르다.
    또한 컬렉션의 크기와 아이템의 개수가 비슷하면 컬렉션을 sorted(items)[:N] 이나 sorted(items)[-N:] 을 이용해서 정렬하고 사용 하는 것이 더 효과적이다.
